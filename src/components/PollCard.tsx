...
export const PollCard = ({ poll }: PollCardProps) => {
  ...
  return (
    <Card
      className={`group hover:shadow-xl transition-all duration-300 border-0 shadow-lg bg-gradient-to-br ${statusConfig.gradient} border-l-4 ${statusConfig.accent} backdrop-blur-sm overflow-hidden relative`}
    >
      ...
      <div className="relative z-10">
        <CardHeader className="pb-3">
          <div className="flex items-start justify-between mb-3">
            <Badge
              className={`${statusConfig.badge} flex items-center gap-1.5 font-medium px-3 py-1.5 shadow-sm`}
            >
              {statusConfig.icon}
              {statusConfig.text}
            </Badge>
            ...
            {timeLeft && status !== "ended" && (
              <div className="text-right">
                <div className="text-xs text-gray-600 font-medium">{getTimerLabel()}</div>
